...
DockPad.Tests/                           Projet xUnit (737 tests) : ActionResult/McpConfig/services d'actions/McpLogService/McpDispatcher/AppPaths
...
| Ouvrir un terminal Codex | FolderBackground | `wt.exe -w 0 new-tab --startingDirectory "%V" -- codex` (décalque du précédent, **sans réglage d'arguments** : celui de Claude existe parce qu'un besoin réel l'a demandé. Icône tirée du **vrai** `codex.exe`, cherché sous le `vendor` du paquet npm — le `PATH` ne porte que `codex.cmd`, un script. Il n'embarque aujourd'hui aucune icône ; on le pointe quand même, pour que celle qu'OpenAI y mettra un jour apparaisse sans toucher au code. Proposé sans condition, comme Claude) |
