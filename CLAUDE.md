...
- **Les quotas sont lus dans `payload.rate_limits`**, par `CodexQuotaReader`, pendant le scan des jetons. Le dernier horodatage d'événement gagne entre sessions et archives. `primary` peut être hebdomadaire : seule `window_minutes` décide (300 → session, 10080 → semaine). Fenêtres expirées masquées, avec notice si aucune jauge ne reste. Aucun processus enfant ni accès aux identifiants.
- **Un relevé de plus de 15 minutes reste affiché, daté** (« 27 % semaine · il y a 1 h », heure exacte en infobulle) — `UsageWindow.ObservedAt`. Il était masqué : Codex n'a pas d'API de quota, le relevé ne se renouvelle **qu'en s'en servant**, et la jauge disparaissait donc un quart d'heure après chaque session, soit la plupart du temps, derrière « Quota Codex indisponible ». Un vieux relevé ne peut que sous-estimer : le chiffre ne monte que si Codex est utilisé. Seule exception : une fenêtre **sans heure de remise à zéro** est masquée une fois ancienne, rien ne dit qu'elle court encore. La règle de Claude (masquer au-delà de 15 min) n'est pas touchée : lui peut relire l'API
- **Le modèle vient du `turn_context`**, pas du `token_count` : c'est la ligne qui ouvre chaque tour et elle seule porte `model`. Il vaut pour les relevés qui le suivent **dans le même fichier**, y compris un tour ouvert avant la borne de scan. La lecture de `info.model` rendait une colonne toujours vide, et la fixture de test inventait ce champ — c'est ce qui cachait le défaut. Aucun tour dans la fenêtre → repli sur `model = …` de la table racine de `config.toml` (`CodexUsageReader.ConfiguredModel`), qui dit ce qu'une session *démarrerait* et non ce qu'un `--model` a fait
...
UsageShot.exe panel-codex-stale ...                             # releve Codex ancien : jauge conservee, datee
...
> dessin. `PresetService.BuildIco` écrit donc le conteneur à la main, avec un **PNG**
> dedans : le shell les lit depuis Vista, et ce format préserve l'alpha exactement là où
> le DIB classique le reconstruit depuis un masque. Vérifié après coup : **0 pixel de
> différence** entre l'icône source et le fichier produit.
